# Pacer DX Review -- Priya Sharma, Principal UX Engineer

## Executive Summary

Pacer solves a real and growing problem. As parallel AI-agent workflows become standard in 2026, git worktree management is now a daily necessity rather than a niche concern. The tool has solid bones: smart defaults, good error messages, and a thoughtful hook architecture. But the first five minutes are rough, the concept surface area is too wide for the value delivered, and the UI layer (promptui) is holding back a tool that wants to feel like lazygit-for-worktrees.
...
```
brew install daniil-lyalko/pacer/pacer   # 1. Install
pacer foo                                 # 2. First run - wizard fires
                                          # 3. Pick AI tool
...
1. User installs pacer via Homebrew. Smooth.

2. User runs `pacer foo` from their home directory (as the README suggests). The first-run wizard fires (lines 169-218 of `/Users/dlyalko/daniil/pacer/internal/config/config.go`), asks a single question about their AI tool, saves config. Good.
...
4. Now the user must:
   ...
   - Run `pacer repo add ~/repos/my-api`
   - Run `pacer foo` again
   - Get prompted for branch name (another interruption)
   - Worktree gets created

**That is 4-5 steps and 2 context switches before the user sees any value.** The README's Quick Start shows `pacer foo` as step one, but if the user is not already `cd`-ed into a git repo, it fails immediately after the wizard completes.
...
Pacer has **four** separate setup/init concepts:

| Command | What it does | When needed |
|---------|-------------|-------------|
| **First-run wizard** (`config.go:169`) | Sets agent/editor preference | Auto on first run |
| **`pacer init`** (`init.go`) | Creates `.pacer/hooks.yaml.example`, updates `.gitignore` | Per-repo, manual |
| **`pacer setup`** (`setup.go`) | Installs global hooks to `~/.claude/`, `~/.cursor/` | One-time, manual |
| **`pacer repo add`** | Registers a repo in config | Per-repo, manual |
...
Additionally, `pacer init` (in `init.go` lines 35-79) now only creates `.pacer/hooks.yaml.example` and updates `.gitignore` -- it no longer creates `.claude/settings.json` or `.cursor/hooks.json`. The global hooks are in `setup`. But the README at line 97 still says "pacer init -- Setup hooks in current repo" and the CLAUDE.md flow diagram says "pacer init" creates `.claude/settings.json`. This documentation mismatch will confuse users.

**The doctor command reveals the gap.** In `doctor.go` lines 757-858, `checkGlobalHooks()` checks for Claude/Cursor hooks at `~/.claude/settings.json` and `~/.cursor/hooks.json`. When missing, it says "not found -- run 'pacer setup'". But a new user who just ran through the wizard has no idea `pacer setup` exists. The wizard does not mention it. The dashboard does not prompt for it.
...
```
  ...
  Done! Run `pacer foo` to create your first worktree.
```
...
- **Real-time** -- no need to re-run `pacer list` after cleanup
...
Pacer's current approach is mostly flat. The `--help` output shows everything. The README shows everything. The dashboard shows everything (Resume, New, Scratch, List, Register repo, Clean up, Config, Doctor, Setup -- nine choices for a brand new user).
...
**Beginner (day 1):**
```
pacer foo                    # Just works. Wizard handles the rest.
pacer list                   # What do I have?
pacer resume foo             # Get back to work
pacer cleanup foo            # Done
```
Four commands. That is the entire surface area for week one.

**Intermediate (week 2+):**
```
pacer foo -t spike           # Learn about types/labels
pacer foo -r my-api          # Multiple repos
pacer scratch notes          # No-git workspace
pacer config                 # Tune settings
```

**Advanced (month 2+):**
```
pacer foo -b custom/branch   # Custom branch names
pacer --experimental project # Multi-repo coordination
...
2. **Help text should be tiered.** `pacer --help` shows the 4 core commands. `pacer --help --all` shows everything. The current help is already pretty good at grouping, but the flat list of flags on the root command is overwhelming.

3. **The work command hides itself** (`workCmd.Hidden = true` in `work.go` line 65) but its flags leak into the root command because root re-declares them. This is actually a clever pattern -- users type `pacer foo` not `pacer work foo` -- but the `--help` output for root shows all 10+ flags, which is intimidating for a new user.
...
Pacer asks users to understand these concepts:
...
**"Worktree" itself is jargon.** Most developers know what a git branch is. Fewer know what a worktree is. Pacer's documentation explains it well, but the moment a user hits `pacer list` and sees "3 worktrees," they may not immediately map that to "3 active branches I'm working on."
...
1. **The typo guard in root.go is excellent but could go further.** Lines 102-104 use Levenshtein distance to catch `pacer injext-context` (a real risk given the dual role of root as both dispatcher and shorthand for `work`). But it only checks against subcommands. If a user types `pacer resum` (off by one from `resume`), it would try to create a worktree named "resum" because there is no close match (edit distance 1 from "resume" which should trigger). Actually, checking the code at line 115, the threshold is `bestDist := 3` meaning it catches distance <= 2. "resum" to "resume" is distance 1, so it would be caught. Good.

2. **No "did you mean?" for flags.** If a user types `pacer foo --typ spike` instead of `--type spike`, cobra will error with a generic message. Consider adding flag suggestions.

3. **Silent failures in hooks.** From `worktree.go` lines 209-228, hook failures are shown as warnings but execution continues. This is the right default, but there is no way for a user to see "hook output" after the fact. A `pacer logs` or `--verbose` mode for hooks would help debugging.
...
When Pacer was designed, it was unique. Now there are direct competitors:
...
Pacer's differentiator is the **context layer** -- dropbags, hook injection, session continuity. None of the competitors do this. But the worktree management piece alone is becoming commoditized.

### Where Pacer Should Double Down

**Context is the moat.** In 2026, the separating factor between AI coding tools is [how well they integrate with your workflow: tool calling, project context, and predictable behavior](https://addyosmani.com/blog/ai-coding-workflow/). Pacer is uniquely positioned to be the **context orchestration layer** for AI coding sessions.

Vision for 2027:

1. **Parallel agent coordination.** Worktrunk's pitch is "run 5-10 agents in parallel." Pacer should support this natively: `pacer foo -t spike --parallel 3` creates the worktree and launches 3 agent sessions with coordinated context. The `project` concept could evolve into this.
...
3. **Team-aware worktrees.** "Alice is working on PROJ-1234 in my-api. Bob just merged fix/PROJ-1235." Pacer already tracks state; extending it to team coordination (via shared state or git notes) would be powerful.
